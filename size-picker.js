...
(function () {
  ...
  proto.setThumbFromPreview = function (size) {
    ...
    var percent = (size - window.app.brushSize.min) / (window.app.brushSize.max - window.app.brushSize.min);
    percent = (percent + 0.1) * percent - 0.1;
    var x = percent * this.rangeWidth;
    var sliderX = Math.min(Math.max(x, 0), this.rangeWidth);
  ...
  proto.onTap = function (e) {
    ...
    var sliderX = Math.min(Math.max(touchX, 0), this.rangeWidth);
  ...
  proto.attachedCallback = function () {
    ...
    this.rangeWidth = this.querySelector('.horizontal-range').getBoundingClientRect().width;
